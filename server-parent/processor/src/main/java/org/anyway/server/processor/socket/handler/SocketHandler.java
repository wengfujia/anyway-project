...
public class SocketHandler extends SimpleDefaultHandler {
     ...
     * @param stream
     */
	private void HandleMsgStream(final ChannelHandlerContext ctx, TcpMessageCoder stream) {
		...
		try {    
			if (commandid == CommandID.TEST) {
		    	...
		    	if (status ==0) {
		    		LoggerUtil.sprintf(log, "[socket]Sucess! CommandID:%s,User:%s,Content:%.10s,IP:%s", String.valueOf(header.getCommandID()), header.getUser(), NetUtil.getString(nr, uConfigVar.CharsetName), clientIP);
		    	} else if (status != -12) {
		    		//查出错误含义
		    		if (list.Count()==0) {
		    		...
		    		LoggerUtil.sprintf(log, "[socket]Fail! ErrorCode:%s， CommandID:%s,User:%s,Content:%.10s,IP:%s", status, header.getCommandID(), header.getUser(), NetUtil.getString(nr, uConfigVar.CharsetName), clientIP);		    		
		    	}
		    }
